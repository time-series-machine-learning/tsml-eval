...
# Mail a progress report for the UCR completion run every 12 hours.
#
...
#
#   bash mail_ucr_completion_progress.sh
#
# Send one report right now and schedule nothing:
#
#   bash mail_ucr_completion_progress.sh --once
#
# Stop the cycle:
#
#   bash mail_ucr_completion_progress.sh --stop
#
...
report_label="completion"
report_job_name="ucr-completion-report"
...
monitor_path="$(dirname "${script_path}")/monitor_ucr_completion.sh"

usage() {
    printf '%s\n' \
        "Usage:" \
        "  mail_ucr_completion_progress.sh [options]" \
        "" \
        "Options:" \
...
send_report() {
    ...
    UCR_RESULTS_ROOT="${results_root}" \
        bash "${monitor_path}" --summary \
    ...
    subject="UCR ${report_label} [${machine}]: ${overall}"
    send_mail "${subject}" "${report_file}" || true

    # Compare the counts rather than the percentage: 47039/47040 rounds to
    # 100.00%, and a run one experiment short is not a run to stop reporting on.
    if [[ "${overall}" =~ ^([0-9]+)/([0-9]+) ]] &&
...
if [[ "${mode}" == "local" ]]; then
    ...
    #   nohup bash mail_ucr_completion_progress.sh --mode local &
    while true; do
        if ! send_report; then
