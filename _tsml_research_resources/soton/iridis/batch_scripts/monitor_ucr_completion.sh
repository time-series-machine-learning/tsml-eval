...
# Monitor the UCR results completion run.
#
# This script is read-only. Run it on Iridis for one snapshot:
#
#   bash monitor_ucr_completion.sh
#
# Include every incomplete estimator/problem pair:
#
#   bash monitor_ucr_completion.sh --details
#
# Refresh every 60 seconds (Ctrl-C to stop):
#
#   bash monitor_ucr_completion.sh --watch 60
#
# Completion is counted from the result files, so it is correct whether or not
# the chain of rounds submitted by run_ucr_completion.sh is still alive. Failure
# attribution comes from that script's attempt state, which is where the memory
# tier of each experiment is recorded.
#
# The estimator table below must stay in step with experiment_spec in
# run_ucr_completion.sh: same estimators, same results subdirectories, and the
# same train-file expectation. An estimator that carries train files is only
# counted complete for a resample when both files are present, which is what the
# runner requires, so the two never disagree about what is left.
...
dataset_list_file="${UCR_COMPLETION_DATASET_LIST:-${local_path}/Code/tsml-eval/_tsml_research_resources/dataset_lists/UnivariateClassification112-UCR2018Clean.txt}"
...
# Must match experiment_spec in run_ucr_completion.sh.
regressors=(
    "BOSS"
    "CBOSS"
    "MrSEQL"
...
declare -A regressor_category=(
    [BOSS]="DictionaryBased"
    [CBOSS]="DictionaryBased"
    [MrSEQL]="DictionaryBased"
    [WEASEL_V2]="DictionaryBased"
    [BaggedQuant]="IntervalBased"
    [RIST]="Hybrid"
    [SharedDrCIF-min]="IntervalBased/TestOnlyExperimental"
    [SharedDrCIF-nopgram]="IntervalBased/TestOnlyExperimental"
    [pulsar-et]="IntervalBased/TestOnlyExperimental"
    [pulsar-noar]="IntervalBased/TestOnlyExperimental"
    [pulsar-nopool]="IntervalBased/TestOnlyExperimental"
    [pulsar-nosel]="IntervalBased/TestOnlyExperimental"
    [pulsar-ridge]="IntervalBased/TestOnlyExperimental"
)

# Estimators whose resamples are only complete with a train file as well.
declare -A regressor_needs_train=(
    [BOSS]=1
    [CBOSS]=1
    [MrSEQL]=1
    [WEASEL_V2]=1
...
usage() {
    printf '%s\n' \
        "Usage:" \
        "  monitor_ucr_completion.sh [--details] [--summary] [--watch SECONDS]" \
        "" \
        "Options:" \
        "  --details        List every incomplete estimator/problem pair." \
...
# One or two finds per estimator rather than one stat per experiment. Zero
# length results are excluded, so a truncated write is not counted as complete.
#
# For an estimator that carries train files, a resample counts only when both
# files are there. That is the runner's own rule, so the two agree on what is
# outstanding rather than the monitor reporting 100% while work remains.
collect_complete_counts() {
    ...
    for regressor in "${regressors[@]}"; do
        ...
        needs_train="${regressor_needs_train[${regressor}]-0}"
        if ((needs_train)); then
            seen_test=()
        ...
        while IFS= read -r path; do
            ...
            if ((needs_train)) && [[ -z "${seen_test["${dataset}|${index}"]-}" ]]; then
                continue
            fi
...
print_relevant_queue() {
    ...
    if ((${#relevant_job_records[@]} == 0)); then
        ...
        echo "  bash run_ucr_completion.sh"
        echo
        return
    fi
...
print_current_activity() {
    ...
    printf '%-22s %-30s %-9s %-12s %s\n' \
        "ESTIMATOR" "DATASET" "RESAMPLE" "JOBID" "NODE"
...
scan_once() {
    ...
    printf 'UCR %s monitor - %s\n' "${workflow_label}" "$(date '+%Y-%m-%d %H:%M:%S %Z')"
    printf 'Machine: %s\n' "$(hostname -f 2>/dev/null || hostname)"
    echo "Results: ${results_root}"
    echo "Scope:   ${#regressors[@]} estimators x ${#datasets[@]} datasets x ${resamples} resamples"
    print_running_nodes
    echo
    ...
    printf '%-22s %14s %9s %8s %8s %8s\n' \
        "ESTIMATOR" "COMPLETE" "PERCENT" "RUNNING" "PENDING" "TODO"
    printf '%-22s %14s %9s %8s %8s %8s\n' \
        "----------------------" "--------------" "---------" "--------" \
        "--------" "--------"
    ...
    for regressor in "${regressors[@]}"; do
        ...
        # Two decimals: one missing resample out of 3360 rounds to 100.0% at
        # one decimal, which reads as finished when it is not.
        printf '%-22s %6d/%-7d %8.2f%% %8d %8d %8d\n' \
            "${regressor}" \
    ...
    echo
    printf 'Overall complete: %d/%d (%.2f%%)\n' \
        "${total_complete}" \
        "${total_expected}" \
    ...
    if [[ "${details}" == "true" ]]; then
        ...
        echo "----------------------------------"
        printf '%-22s %-30s %s\n' "ESTIMATOR" "DATASET" "RESAMPLES DONE"
