...
# Fill the multiverse core results, so that each of the ten Table 2 classifiers
# has 30 resamples on all 66 problems of the multiverse core collection.
#
# Scope was taken from an inventory of Results/Multiverse. Three classifiers are
# already complete on the core 66; the other seven are short by 2,364
# experiments in total:
#
#   TSF          1    AustraliaRainfall_disc, one resample
#   STSF        60    AustraliaRainfall_disc and PenDigits, never run
#   r-STSF     124    4 problems never run, 4 one resample short
#   PULSAR     164    31 problems a few resamples short, none absent
#   HC2        323    4 never run, 7 stuck at 1/30
#   MR-Hydra   409    3 never run, 11 stuck at 1/30
#   RISE      1283    42 of 66 problems never run
#
#   complete: QUANT, DrCIF, CIF (1980/1980 each)
#
# The gaps have three shapes and they matter. RISE is barely started, and is
# more than half the total on its own. HC2 and MR-Hydra look like a resample
# sweep that stopped: a few problems absent, then a block sitting at exactly one
# resample, and those are the expensive problems (FaceDetection, EigenWorms,
# Skoda, CrowdSourced, FordChallenge, the BIDMC pair), so expect them to need the
# higher memory tiers. PULSAR is scattered attrition with nothing wholly absent,
# which reads as intermittent failure rather than a stopped run.
#
# RISE first, separately. RISE is both the largest block and the cheapest per
# experiment, so running it on its own gets the missing half of the archive back
# quickly without queueing behind HC2 fits that take hours:
#
#   bash run_multiverse_core_completion.sh --only RISE
#   bash run_multiverse_core_completion.sh --exclude RISE
#
# Both share one state directory and one job name prefix, so the second sees the
# first's node jobs through squeue and the four node limit still holds if they
# overlap.
#
# Nothing is recomputed. Reconciliation reads the results tree every round and
# submits only the (estimator, dataset, resample) triples whose files are absent,
# so declaring the full 10 x 66 x 30 grid is safe: the 17,436 experiments that
# already exist are skipped.
#
# ---------------------------------------------------------------------------
#
# Estimators and results locations. Each keeps the directory its results already
# occupy, which is what lets one run fill gaps spread across IntervalBased,
# Hybrid and ConvolutionBased. Every multivariate result here is test only, so
# no estimator generates train files; note the directory is MRHydra, with a
# capital R, unlike the UCR tree's MrHydra.
#
# CIF and DrCIF are the 200 tree aeon defaults, matching the univariate table,
# not the CIF-500 and DrCIF-500 directories.
#
...
#
# Rounds and recovery. A single 60 hour allocation cannot finish the heavier
# experiments here, so this script is built to run repeatedly. Each invocation
#
...
# Usage:
#
#   bash run_multiverse_core_completion.sh --only RISE --dry-run
#   bash run_multiverse_core_completion.sh --only RISE
#   bash run_multiverse_core_completion.sh --exclude RISE --dry-run
#   bash run_multiverse_core_completion.sh --exclude RISE
#   bash run_multiverse_core_completion.sh --no-chain      # one round, no chain
#
# Run --dry-run before each and check the pending count: 1,283 for --only RISE
# and 1,081 for --exclude RISE, 2,364 for the whole set. Those are the gaps
# measured on the results archive; if a dry run reports far more, the results
# tree on this cluster is not the one that was inventoried and the run would
# recompute work that already exists elsewhere.
#
...
#
#   node_memory_budget_gib=450 bash run_multiverse_core_completion.sh
...
# The multiverse core spans a wide size range, so most problems open at 4 GiB and only the
...
job_name_prefix="mv-core-completion"
submission_label="MVCoreCompletion"
workflow_label="multiverse core completion"
...
experiment_spec=(
    "RISE|IntervalBased|no"
    "TSF|IntervalBased|no"
    "STSF|IntervalBased|no"
    "CIF|IntervalBased|no"
    "DrCIF|IntervalBased|no"
    "RSTSF|IntervalBased|no"
    "QUANT|IntervalBased|no"
    "PULSAR|IntervalBased|no"
    "HC2|Hybrid|no"
    "MRHydra|ConvolutionBased|no"
)
...
# The branch that registers all ten estimators. This is a warning rather
...
# actually matters.
expected_branch="${expected_branch:-ajb/hc2}"
...
# Multiverse datasets live one directory per problem under here:
...
results_root="${MV_RESULTS_ROOT:-${local_path}/Results/Multiverse}"
out_dir="${results_root}/output-completion"
state_dir="${results_root}/.mv-core-completion-state"
numba_cache_dir="${local_path}/Code/.cache/${env_name}"
shared_runner_lock="${MV_COMPLETION_RUNNER_LOCK:-${results_root}/.mv-core-completion-runner.lock}"
...
usage() {
    printf '%s\n' \
        "Usage:" \
        "  run_multiverse_core_completion.sh [options]" \
        "" \
        "Options:" \
        ...
        "  --only NAMES         Comma separated estimators to run, others" \
        "                       are left alone. Use to run RISE on its own." \
        "  --exclude NAMES      Comma separated estimators to leave out." \
...
while (($# > 0)); do
    case "$1" in
        ...
        --exclude)
            if (($# < 2)); then
                echo "ERROR: --exclude requires a comma separated list." >&2
                exit 2
            fi
            exclude_classifiers="$2"
            shift 2
            ;;
        --skip-unresolved)
...
if [[ -n "${only_classifiers}" && -n "${exclude_classifiers}" ]]; then
    echo "ERROR: --only and --exclude cannot be combined." >&2
    exit 2
fi

# Selecting a subset only narrows what this invocation declares. The estimators
# left out keep their existing results and their gaps; a later invocation without
# the filter picks them up. Matching is case insensitive so --only rise works.
filter_classifiers() {
    local wanted="$1"
    local mode="$2"
    local -a requested=()
    ...
    for entry in "${requested[@]}"; do
        ...
        for classifier in "${classifiers[@]}"; do
            if [[ "${classifier,,}" == "${entry,,}" ]]; then
                match="true"
                break
            fi
        done
        if [[ "${match}" == "false" ]]; then
            echo "ERROR: --${mode} names an estimator that is not in the spec: ${entry}" >&2
            printf 'Known estimators: %s\n' "${classifiers[*]}" >&2
            exit 2
        fi
    done
    ...
    for classifier in "${classifiers[@]}"; do
        ...
        for entry in "${requested[@]}"; do
            entry="${entry//[[:space:]]/}"
            if [[ -n "${entry}" && "${classifier,,}" == "${entry,,}" ]]; then
                match="true"
                break
            fi
        done
        if [[ ("${mode}" == "only" && "${match}" == "true") ||
              ("${mode}" == "exclude" && "${match}" == "false") ]]; then
            kept+=("${classifier}")
        else
    ...
    if ((${#kept[@]} == 0)); then
        echo "ERROR: --${mode} left no estimators to run." >&2
        exit 2
    fi
    classifiers=("${kept[@]}")
}

if [[ -n "${only_classifiers}" ]]; then
    filter_classifiers "${only_classifiers}" "only"
elif [[ -n "${exclude_classifiers}" ]]; then
    filter_classifiers "${exclude_classifiers}" "exclude"
fi
...
if [[ -n "${unresolved_report}" ]]; then
    ...
    echo "aeon soft dependencies are the usual cause."
    echo
...
if ((${#missing_data[@]} > 0)); then
    ...
    printf '  %s\n' "${missing_data[@]}"
    echo "Set data_dir=... if the Multiverse archive lives elsewhere on the cluster."
    exit 1
fi
...
echo "Multiverse ${workflow_label} run - round ${round} of at most ${max_rounds}"
...
send_round_summary() {
    ...
    subject="Multiverse ${workflow_label} round ${round}: ${percent}% complete, ${#pending_keys[@]} left"
...
if [[ "${chain}" != "true" ]]; then
    ...
    if [[ -n "${only_classifiers}" ]]; then
        successor_skip_arg+=" \\
    --only ${only_classifiers}"
    elif [[ -n "${exclude_classifiers}" ]]; then
        successor_skip_arg+=" \\
    --exclude ${exclude_classifiers}"
    fi
