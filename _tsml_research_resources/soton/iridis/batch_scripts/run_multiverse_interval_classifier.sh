...
# (MultivariateIntervalMissing-<MV_CLASSIFIER>.txt) so complete datasets are not
# re-run. Results land in Results/Multiverse/IntervalBased/<MV_CLASSIFIER>,
# matching the store the hali controller produced, ready to sync back. Test only,
# like the rest of the multivariate interval results.
#
# Usage:
#   MV_CLASSIFIER=RISE  bash run_multiverse_interval_classifier.sh
#   MV_CLASSIFIER=RSTSF bash run_multiverse_interval_classifier.sh
#   MV_CLASSIFIER=STSF  bash run_multiverse_interval_classifier.sh
#   MV_CLASSIFIER=TSF   bash run_multiverse_interval_classifier.sh
# Pass --dataset-list <file> to run a different set (e.g. the full 66-core).
#
...
# Usage:
#
#   bash run_multiverse_interval_classifier.sh                 # start the run
#   bash run_multiverse_interval_classifier.sh --dry-run       # show the plan only
#   bash run_multiverse_interval_classifier.sh --no-chain      # one round, no chain
#   bash run_multiverse_interval_classifier.sh --round 2 --no-chain
...
#
#   node_memory_budget_gib=450 bash run_multiverse_interval_classifier.sh
...
# The Multiverse archive is mostly small, so most problems open at 4 GiB and only the
...
# rule guesses from the raw file size, and a guess that opens too low costs a
# whole round: the experiment is killed, escalated one tier, and only retried
# next time round. When a previous run has already shown what the work needs, it
# is cheaper to open there and lose parallelism than to climb a tier at a time.
# Tier 5 is 64 GiB and tier 6 is 128 GiB.
#
#   MV_CLASSIFIER=RSTSF min_start_tier=5 bash run_multiverse_interval_classifier.sh
#
# Unlike default_dataset_start_tier, which only moves the small-dataset default
# and can leave the large datasets opening lower than the small ones, this
# raises every dataset that would otherwise start below it.
...
# These are stock aeon interval classifiers resolved through tsml-eval's
# get_classifier_by_name (_get_classifier.py). aeon supplies the estimator; the
# branch only needs the registration and this runner. The run is pinned to a
# stable branch so the estimator definition does not drift across resamples.
...
usage() {
    printf '%s\n' \
        "Usage:" \
        "  run_multiverse_interval_classifier.sh [options]" \
        "" \
        "Options:" \
...
# aeon or registration definitions across resamples.
...
if [[ -f "${pinned_source_file}" ]]; then
    ...
    printf 'aeon %s\nsource %s\n' \
        "${aeon_head}" "${source_hash}" > "${pinned_source_file}"
...
echo "Opening tier:      ${min_start_tier} (${memory_tiers_gib[min_start_tier - 1]} GiB floor, from min_start_tier)"
