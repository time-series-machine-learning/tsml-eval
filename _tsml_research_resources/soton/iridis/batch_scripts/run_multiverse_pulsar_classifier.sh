...
# aeon interval forests use, so the feature count stays independent of the number
# of channels and even the very high channel counts in this archive (PEMS-SF,
# DuckDuckGeese) fit cheaply. Results are test only, matching the other interval
# classifiers already under Results/Multiverse/IntervalBased; no train files are
# generated here (flip generate_train_files below if a CAWPE weight is needed for
# a multivariate HC2 swap later).
#
...
job_name_prefix="pulsar-mv"
submission_label="MVPulsar"
...
results_dir="${MV_PULSAR_RESULTS_ROOT:-${local_path}/Results/Multiverse/IntervalBased}"
out_dir="${results_dir}/output"
state_dir="${results_dir}/.mv-pulsar-state"
