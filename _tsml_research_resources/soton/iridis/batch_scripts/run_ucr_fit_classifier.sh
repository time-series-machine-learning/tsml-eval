...
# Run the FIT interval classifier over the 112 problem UCR archive, writing test
# prediction files.
#
# Scope: 1 classifier x 112 datasets x 30 resamples = 3360 experiments.
#
# FIT is a from-paper port in tsml_eval/_wip/classification/_fit.py (Li, Xu,
# Wang & Yu, Expert Systems With Applications 213, 2023). It is a standalone
# comparator for the interval survey, not an HC2 component, so this run is test
# only. FIT is already heavy: on top of the forest it runs a 5x10-fold
# cross-validation to select representations and features, so it costs roughly
# ten times TSF per fit. Writing train files (generate_train_files=true) would
# wrap that in a further 10-fold cross_val_predict for ~11x the cost, and is not
# needed for the bake-off.
#
...
# Usage:
#
#   bash run_ucr_fit_classifier.sh                 # start the run
#   bash run_ucr_fit_classifier.sh --dry-run       # show the plan only
#   bash run_ucr_fit_classifier.sh --no-chain      # one round, no chain
#   bash run_ucr_fit_classifier.sh --round 2 --no-chain
...
#
#   node_memory_budget_gib=450 bash run_ucr_fit_classifier.sh
...
job_name_prefix="fit-interval"
submission_label="UCRFit"
workflow_label="FIT classifier"

# Test only: FIT is a standalone comparator, not an HC2 component, so no CAWPE
# train estimate is needed. Set to "true" to also write train files (~11x cost).
...
# FIT lives in tsml-eval, so this branch must carry _fit.py and its registration
# in _get_classifier.py. FIT builds its representations (ACF, DFT, periodogram)
# with numpy and sklearn directly, so any recent aeon that provides
# BaseClassifier is sufficient.
...
results_dir="${UCR_FIT_RESULTS_ROOT:-${local_path}/Results/UCR/IntervalBased}"
out_dir="${results_dir}/output"
state_dir="${results_dir}/.ucr-fit-state"
numba_cache_dir="${local_path}/Code/.cache/${env_name}"
shared_runner_lock="${UCR_FIT_RUNNER_LOCK:-${local_path}/Results/UCR/.ucr-fit-runner.lock}"
...
usage() {
    printf '%s\n' \
        "Usage:" \
        "  run_ucr_fit_classifier.sh [options]" \
        "" \
        "Options:" \
...
if [[ "${current_branch}" != "${expected_branch}" ]]; then
    ...
    echo "That branch must carry the FIT implementation and its registration."
    exit 1
fi
...
# FIT or aeon definitions across resamples.
tsml_eval_head=$(git -C "${tsml_eval_dir}" rev-parse HEAD)
aeon_head=$(git -C "${aeon_dir}" rev-parse HEAD)
fit_source_files=(
    "${tsml_eval_dir}/tsml_eval/_wip/classification/_fit.py"
    "${tsml_eval_dir}/tsml_eval/experiments/_get_classifier.py"
)
for source_file in "${fit_source_files[@]}"; do
    if [[ ! -s "${source_file}" ]]; then
        echo "ERROR: FIT source file is missing or empty: ${source_file}" >&2
        exit 1
    fi
done
fit_source_hash=$(sha256sum "${fit_source_files[@]}" | sha256sum | cut -d ' ' -f 1)
...
if [[ -f "${pinned_source_file}" ]]; then
    ...
    printf 'aeon %s\nfit %s\n' \
        "${aeon_head}" "${fit_source_hash}" > "${pinned_source_file}"
    pinned_aeon_commit="${aeon_head}"
    pinned_fit_hash="${fit_source_hash}"
fi
