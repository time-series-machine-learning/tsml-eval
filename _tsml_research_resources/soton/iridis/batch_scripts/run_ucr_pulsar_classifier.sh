...
#
# That chaining is what makes the run unattended.
#
# Usage:
#
...
if ((${#chain_dependency_job_ids[@]} == 0 &&
      ${#pending_keys[@]} > 0 &&
      ${#occupied_job_ids[@]} > 0)); then
    # No slot was free. Wake the chain when a currently occupied allocation ends.
    chain_dependency_job_ids=("${occupied_job_ids[0]}")
fi
