#!/bin/bash
# Start the missing MultiverseCore LITETime-MV resample-0 jobs on Hali GPUs.

set -euo pipefail

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
repo_dir=$(cd -- "${script_dir}/.." && pwd)
config_file="${script_dir}/multiverse_core_resample0_litemv_gpu.toml"
supervisor="${script_dir}/run_multiverse_controller.sh"
state_dir="/gpfs/home/${USER}/Results/Multiverse/.controller-core-resample0-litemv-gpu"
session_name="multiverse-litemv-gpu"
...
echo "Stopping an earlier LITETime-MV GPU controller, if present."
...
echo "Starting detached LITETime-MV GPU controller: ${session_name}"
...
echo
echo "LITETime-MV GPU controller started. AustraliaRainfall_disc and Tiselac are excluded."
