#!/bin/bash
# Start the resample-0 Multiverse-Core DrCIF-500 train-file controller on HALI.

set -euo pipefail

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
repo_dir=$(cd -- "${script_dir}/.." && pwd)
config_file="${script_dir}/multiverse_core_resample0_drcif_train.toml"
supervisor="${script_dir}/run_multiverse_controller.sh"
session_name="multiverse-core-drcif-train"
required_branch="ajb/hc2"
state_dir="/gpfs/home/${USER}/Results/Multiverse/.controller-core-resample0-drcif-train"
reset_state=false
...
echo "Stopping an earlier DrCIF train-file controller on this login node, if present."
pkill -TERM -f '[r]un_multiverse_controller.sh.*multiverse_core_resample0_drcif_train.toml' || true
pkill -TERM -f '[m]ultiverse_controller.py.*multiverse_core_resample0_drcif_train.toml' || true
...
echo "Checking missing Multiverse-Core DrCIF-500 train/test results."
...
echo
echo "DrCIF-500 train-file controller started successfully."
