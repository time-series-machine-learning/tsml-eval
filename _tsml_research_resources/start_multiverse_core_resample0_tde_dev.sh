#!/bin/bash
# Start the resample-0 Multiverse-Core TDE_Dev controller on HALI.

set -euo pipefail

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
repo_dir=$(cd -- "${script_dir}/.." && pwd)
config_file="${script_dir}/multiverse_core_resample0_tde_dev.toml"
supervisor="${script_dir}/run_multiverse_controller.sh"
session_name="multiverse-core-tde-dev"
required_branch="ajb/hc2"
state_dir="/gpfs/home/${USER}/Results/Multiverse/.controller-core-resample0-tde-dev"
reset_state=false
...
echo "Stopping an earlier TDE_Dev controller on this login node, if present."
pkill -TERM -f "[r]un_multiverse_controller.sh.*multiverse_core_resample0_tde_dev.toml" || true
pkill -TERM -f "[m]ultiverse_controller.py.*multiverse_core_resample0_tde_dev.toml" || true
...
echo "Checking the missing Multiverse-Core TDE_Dev work."
...
echo
echo "TDE_Dev controller started successfully."
