#!/bin/bash
# Run HC2 on resample 0 without train-file generation.
...
config_file="${script_dir}/multiverse_hc2_resample0_no_train.toml"
session_name="multiverse-hc2-resample0"
state_dir="/gpfs/home/${USER}/Results/Multiverse/.controller-hc2-resample0-no-train"
...
if [[ ! -f "${supervisor}" || ! -f "${config_file}" ]]; then
    echo "ERROR: supervisor or HC2 configuration is missing." >&2
    exit 1
fi
...
echo "Starting HC2 controller session: ${session_name}"
screen -dmS "${session_name}" bash -lc '
    ...
    echo "HC2 resample-0 run completed."
'

sleep 2
if ! screen -ls | grep -Fq ".${session_name}"; then
    echo "ERROR: the detached HC2 controller session did not remain running." >&2
    exit 1
fi
