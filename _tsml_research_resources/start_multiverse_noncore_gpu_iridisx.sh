#!/bin/bash
# Start both non-core GPU controllers on IridisX's Early Access H200 partition.
#
# Extends resample 0 beyond the core 66 to the 23 datasets outside it that already
# have data and are not covered elsewhere. Of the 73 non-core datasets, 32 are
# downloaded; three of those are the UEA sets the uea_completion starter handles, and
# six have no results from any estimator at all, needing a full-archive pass rather
# than a GPU one.
#
# 141 experiments. ConvTran, DisjointCNN, PatchMTSC, TimesNet, TimesURL and TS2Vec are
# missing on all 23; H-InceptionTime on 3; LITETime-MV already has every one.
#
# None of this reaches the leaderboard on its own. All 23 are also missing STSF,
# Summary and TSF, which never had a non-core pass, so the CPU side has to follow or
# the datasets stay outside the scored set.
#
# Two controllers because gpu_check is per configuration: a torch check would pass
# while TensorFlow saw no GPU.
...
dataset_list="${script_dir}/dataset_lists/MultivariateClassification23-NonCoreGPUReady.txt"

# config : state directory suffix
configs=(
    "multiverse_noncore_resample0_deep_torch_gpu_iridisx_i7_h200.toml:deep-torch"
    "multiverse_noncore_resample0_deep_keras_gpu_iridisx_i7_h200.toml:deep-keras"
)
...
if [[ "$(git -C "$repo_dir" branch --show-current)" != "$required_branch" ]]; then
    echo "ERROR: non-core GPU jobs must run from ${required_branch}." >&2
    exit 1
fi
...
# These are not in MultiverseCore, so confirm the data is present AND loads the way
...
echo "Checking every dataset loads, and as equal length."
...
for entry in "${configs[@]}"; do
    ...
    state_dir="${results_dir}/.controller-noncore-resample0-${entry##*:}-i7-h200"
...
echo
echo "Both non-core GPU controllers are running."
