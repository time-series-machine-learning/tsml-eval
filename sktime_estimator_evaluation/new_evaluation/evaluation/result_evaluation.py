...
CLASSIFICATION_METRIC_CALLABLES = [
    ...
    MetricCallable(name="ROC AUC", callable=roc_auc_score),
    MetricCallable(name="Brier", callable=brier_score_loss),
    MetricCallable(name="Log Loss", callable=log_loss),
    MetricCallable(name="Balanced Accuracy", callable=balanced_accuracy_score),
    MetricCallable(name="Top-k Accuracy", callable=top_k_accuracy_score),
    MetricCallable(name="Average Precision", callable=average_precision_score),
]
...
def evaluate_results(
    ...
    metrics: List[Union[MetricCallable, str]], defaults = None
        List of metrics to evaluate. If None, then only accuracy used.
    output_dir: str
