"""FIRERegressor: Fast Interval Representation Ensemble for regression.

The regression counterpart of ``FIRE`` and the consolidated successor to
DrCIFRegressor. It keeps DrCIF's three representations (base series, first
differences, periodogram) and its random interval-length model, but replaces
the per-tree interval forest with a shared-transform architecture and an
extensible ensemble of regressor heads.

Transform (computed ONCE, not per tree):
  * random-length CONTIGUOUS intervals drawn once (no dilation — dilation is a
    receptive-field/convolutional idea off-paradigm for interval regressors,
    and it was accuracy-neutral in classification testing);
...
Heads (averaged): an ExtraTreesRegressor and a scaled Ridge
(StandardScaler + RidgeCV). Predictions from each head are averaged with equal
weight. The ``heads`` list is the extension point: further heads can be
appended and are averaged in.
"""

__maintainer__ = ["TonyBagnall"]
__all__ = ["FIRERegressor"]
...
from tsml_eval._wip.regression._shared_drcif_regressor import SharedDrCIFRegressor

_VALID_HEADS = ("extratrees", "ridge")


class FIRERegressor(SharedDrCIFRegressor):
    """Fast Interval Representation Ensemble regressor.

    Random shared intervals on three representations, a lean feature pool,
    length gating and a constant-feature filter, predicted by an averaged
    ensemble of heads (ExtraTrees + scaled Ridge by default). The settled
    architecture is fixed; the parameters below remain tunable. See
    SharedDrCIFRegressor for the transform parameter and attribute
    documentation.
    ...
    heads : tuple of str, default=("extratrees", "ridge")
        Regressor heads to average, from {"extratrees", "ridge"}. Extension
    ...
    def _fit_head(self, name, Xk, y):
        """Fit one regressor head on the (kept) feature matrix."""
        if name == "extratrees":
    ...
    def _predict(self, X) -> np.ndarray:
        ...
        preds = [np.asarray(h.predict(Xk)) for h in self._heads_.values()]
