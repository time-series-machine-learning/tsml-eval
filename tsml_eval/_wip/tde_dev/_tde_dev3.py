"""Experimental per-dimension similarity fusion for TDE.

Dictionary-based classifiers using the Symbolic Fourier Approximation transform.

This module is an isolated development clone of
``aeon.classification.dictionary_based._tde`` at aeon commit
...
def _dimension_bag_masses(counts, offsets, starts):
    """Return total word-count mass for every dimension and case."""
    n_dims = offsets.shape[0]
    n_cases = offsets.shape[1] - 1
    masses = np.zeros((n_dims, n_cases), dtype=np.float64)

    for d in range(n_dims):
        for n in range(n_cases):
            start = starts[d] + offsets[d, n]
            end = starts[d] + offsets[d, n + 1]
            mass = 0.0
...
@njit(cache=True, nogil=True)
def normalised_histogram_intersection(
    keys1,
    keys2,
    counts,
    start,
    ...
    """Histogram intersection divided by the smaller bag mass.

    Empty bags contribute zero. For non-negative counts the result is in ``[0, 1]``.
    The two bags may be slices of different backing arrays.
    """
    denominator = min(mass, other_mass)
    if denominator <= 0:
        return 0.0

    intersection = 0.0
    i, j = start, other_start
    while i < end and j < other_end:
        key1, key2 = keys1[i], keys2[i]
        other_key1, other_key2 = other_keys1[j], other_keys2[j]
        if key1 == other_key1 and key2 == other_key2:
            intersection += min(counts[i], other_counts[j])
            i += 1
            j += 1
        elif key1 < other_key1 or (key1 == other_key1 and key2 < other_key2):
            i += 1
        else:
            j += 1
...
@njit(cache=True, nogil=True)
def normalised_dimension_similarity(
    keys1,
    keys2,
    counts,
    offsets,
    ...
    """Weighted sum of separately normalised per-dimension similarities."""
    similarity = 0.0
    for d in range(offsets.shape[0]):
        start = starts[d] + offsets[d, case]
        end = starts[d] + offsets[d, case + 1]
        other_start = other_starts[d] + other_offsets[d, other_case]
        other_end = other_starts[d] + other_offsets[d, other_case + 1]
        similarity += weights[d] * normalised_histogram_intersection(
            keys1,
            keys2,
            counts,
            start,
            ...
            other_start,
            other_end,
            other_masses[d, other_case],
        )
    return similarity


@njit(cache=True, nogil=True)
def _normalised_dimension_similarities_all(
    keys1,
    keys2,
    counts,
    offsets,
    ...
    """Score every test case against every train case inside Numba."""
    n_train = offsets.shape[1] - 1
    n_test = test_offsets.shape[1] - 1
    similarities = np.zeros((n_test, n_train), dtype=np.float64)

    for test_case in range(n_test):
        for train_case in range(n_train):
            similarities[test_case, train_case] = normalised_dimension_similarity(
                test_keys1,
                test_keys2,
                test_counts,
                test_offsets,
...
@njit(cache=True)
def _normalised_dimension_loocv_train_acc(
    keys1,
    keys2,
    counts,
    offsets,
    ...
    """LOOCV 1NN using weighted, normalised per-dimension similarity."""
    n_cases = offsets.shape[1] - 1
    similarities = np.zeros((n_cases, n_cases), dtype=np.float64)
    predictions = np.full(n_cases, -1, dtype=np.int64)
    correct = 0

    for i in range(n_cases):
        if correct + n_cases - i < required_correct:
        ...
        predictions[i] = nearest_neighbour
        if y_codes[nearest_neighbour] == y_codes[i]:
            correct += 1

    return n_cases, correct, predictions


@njit(cache=True, nogil=True)
def _normalised_dimension_predict_loocv(
    keys1, keys2, counts, offsets, starts, masses, weights, train_case
):
    """Return the leave-one-out 1NN index for one train case."""
    best_similarity = -1.0
    nearest_neighbour = -1
    for other_case in range(offsets.shape[1] - 1):
        ...
        if similarity > best_similarity:
            best_similarity = similarity
            nearest_neighbour = other_case
    return nearest_neighbour


class TDE_Dev3(BaseClassifier):
    """Temporal Dictionary Ensemble (TDE).

    TDE evaluates parameter settings for ``IndividualTDE_Dev3`` classifiers and
    retains an accuracy-weighted ensemble, as described in [1]_. After an initial
    random search, kernel ridge regression fitted to previously evaluated settings
    guides parameter selection. The reference paper describes this surrogate model as
    ...
    multivariate_similarity : {"merged", "normalised"}, default="merged"
        How histogram-intersection evidence is combined across selected dimensions.
        ``"merged"`` preserves the standard TDE behaviour. ``"normalised"``
        computes intersection separately for each dimension, divides it by the
        smaller of the two bag masses, and combines the resulting similarities.
        This parameter has no effect for univariate data.
    dimension_weighting : {"uniform", "accuracy"}, default="uniform"
        Weights used to combine dimensions in ``"normalised"`` mode. Accuracy
        weights use the training-only univariate leave-one-out accuracies already
        calculated by TDE's dimension selection. This parameter has no effect in
        ``"merged"`` mode or for univariate data.
    ...
    estimators_ : list of IndividualTDE_Dev3
    ...
    See Also
    --------
    IndividualTDE_Dev3, ContractableBOSS
    ...
    Examples
    --------
    >>> from aeon.classification.dictionary_based import TDE_Dev3
    ...
    def __init__(
        ...
        self.multivariate_similarity = multivariate_similarity
        self.dimension_weighting = dimension_weighting
    ...
    def _fit(self, X, y, keep_train_preds=False):
        ...
        if self.n_parameter_samples <= self.randomly_selected_params:
            warnings.warn(
                "TDE_Dev3 warning: n_parameter_samples <= "
        ...
        while (
            ...
            # oversubscribe
            tde = IndividualTDE_Dev3(
                *parameters,
                bigrams=use_bigrams,
                dim_threshold=self.dim_threshold,
                max_dims=self.max_dims,
                multivariate_similarity=self.multivariate_similarity,
                dimension_weighting=self.dimension_weighting,
    ...
    def _individual_train_acc(self, tde, y, train_size, lowest_acc, keep_train_preds):
        ...
        if train_size <= _SYMMETRIC_LOOCV_MAX_N:
            ...
            if (
                tde.n_channels_ > 1
                and tde.multivariate_similarity == "normalised"
            ):
                n_done, correct, preds = _normalised_dimension_loocv_train_acc(
                    *tde._transformed_data,
                    tde._dimension_weights_,
    ...
    def _get_test_params(cls, parameter_set="default"):
        ...
        parameter_set : str, default="default"
            ...
            TDE_Dev3 provides the following special sets:
...
class IndividualTDE_Dev3(BaseClassifier):
    """Single TDE classifier extending the Bag of SFA Symbols (BOSS) model.

    This is the base classifier used by ``TDE_Dev3``. It applies an
    ...
    max_dims : int, default=20
        ...
        multivariate data.
    multivariate_similarity : {"merged", "normalised"}, default="merged"
        How selected-dimension histogram intersections are combined. ``"merged"``
        is standard TDE. ``"normalised"`` scores each dimension separately after
        dividing intersection by the smaller bag mass.
    dimension_weighting : {"uniform", "accuracy"}, default="uniform"
        In normalised mode, either average dimensions uniformly or weight them by
        their training-only univariate leave-one-out dimension-selection accuracy.
    typed_dict : bool or str, default="deprecated"
        Has no effect: word counts are now stored as sorted arrays.
    ...
    For the Java version, see
    `TSML <https://github.com/uea-machine-learning/tsml/blob/master/src/main/java/
    tsml/classifiers/dictionary_based/IndividualTDE_Dev3.java>`_.
    ...
    Examples
    --------
    >>> from aeon.classification.dictionary_based import IndividualTDE_Dev3
    >>> from aeon.datasets import load_unit_test
    >>> X_train, y_train = load_unit_test(split="train")
    >>> X_test, y_test = load_unit_test(split="test")
    >>> clf = IndividualTDE_Dev3()
    >>> clf.fit(X_train, y_train)
    IndividualTDE_Dev3(...)
    >>> y_pred = clf.predict(X_test)
    """
    ...
    def __init__(
        ...
        self._dimension_scores_ = np.empty(0, dtype=np.float64)
        self._dimension_weights_ = np.empty(0, dtype=np.float64)
    ...
    def _fit(self, X, y):
        ...
        valid_similarities = {"merged", "normalised"}
        if self.multivariate_similarity not in valid_similarities:
            raise ValueError(
                "multivariate_similarity must be one of "
                f"{valid_similarities}, but found {self.multivariate_similarity!r}."
            )
        valid_weightings = {"uniform", "accuracy"}
        if self.dimension_weighting not in valid_weightings:
            raise ValueError(
                "dimension_weighting must be one of "
                f"{valid_weightings}, but found {self.dimension_weighting!r}."
            )
        ...
        if self.n_channels_ > 1:
            ...
            if self.multivariate_similarity == "normalised":
                self._transformed_data = self._stack_dim_bags(dim_words)
            else:
    ...
    def _predict(self, X):
        ...
        if self.n_channels_ > 1:
            ...
            if self.multivariate_similarity == "normalised":
                test_bags = self._stack_dim_bags(dim_words)
            else:
        ...
        if self.n_channels_ > 1 and self.multivariate_similarity == "normalised":
            keys1, keys2, counts, t_offsets, t_starts, t_masses = test_bags
            if self._n_jobs > 1 and n_cases > 1:
                chunks = np.array_split(
                    np.arange(n_cases), min(self._n_jobs, n_cases)
                )
                sims = np.vstack(
                    Parallel(n_jobs=self._n_jobs, prefer="threads")(
                        delayed(_normalised_dimension_similarities_all)(
                            *self._transformed_data,
                            keys1,
                            keys2,
                            counts,
                            ...
                            t_starts,
                            t_masses[:, chunk],
                            self._dimension_weights_,
                        )
                        for chunk in chunks
                    )
                )
            else:
                sims = _normalised_dimension_similarities_all(
                    *self._transformed_data,
                    *test_bags,
                    self._dimension_weights_,
                )
    ...
    def _stack_dim_bags(self, dim_bags):
        """Store selected-dimension bags without tagging and merging their words."""
        all_k1 = np.concatenate([bag[0] for bag in dim_bags])
        all_k2 = np.concatenate([bag[1] for bag in dim_bags])
        all_counts = np.concatenate([bag[2] for bag in dim_bags])
        dim_case_offsets = np.vstack([bag[3] for bag in dim_bags])
        sizes = np.asarray([len(bag[0]) for bag in dim_bags], dtype=np.int64)
        dim_starts = np.zeros(len(dim_bags), dtype=np.int64)
        dim_starts[1:] = np.cumsum(sizes)[:-1]
        masses = _dimension_bag_masses(all_counts, dim_case_offsets, dim_starts)
        return (
            all_k1,
            all_k2,
            all_counts,
    ...
    def _select_dims(self, X, y):
        ...
        if len(dims) > self.max_dims:
            ...
            selected_accs = [selected_accs[i] for i in idx]

        self._dimension_scores_ = np.asarray(selected_accs, dtype=np.float64)
        self._dimension_scores_ /= self.n_cases_
        if self.dimension_weighting == "accuracy":
            weight_sum = self._dimension_scores_.sum()
            if weight_sum > 0:
                self._dimension_weights_ = self._dimension_scores_ / weight_sum
            else:
                self._dimension_weights_ = np.full(
                    len(dims), 1.0 / len(dims), dtype=np.float64
                )
        else:
            self._dimension_weights_ = np.full(
                len(dims), 1.0 / len(dims), dtype=np.float64
    ...
    def _train_predict(self, train_num, bags=None):
        supplied_bags = bags is not None
        if bags is None:
            bags = self._transformed_data

        if (
            not supplied_bags
            and self.n_channels_ > 1
            and self.multivariate_similarity == "normalised"
        ):
            nn_idx = _normalised_dimension_predict_loocv(
                *bags, self._dimension_weights_, train_num
            )
        else:
