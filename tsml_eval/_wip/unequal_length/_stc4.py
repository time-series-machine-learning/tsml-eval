...
from tsml_eval._wip.unequal_length.other._shapelet_transform3 import \
...
class ShapeletTransformClassifier(BaseClassifier):
    ...
    def _predict(self, X) -> np.ndarray:
        ...
        X_t = np.nan_to_num(X_t, False, np.finfo(np.float32).max, np.finfo(np.float32).max, np.finfo(np.float32).max)
    ...
    def _fit_stc_shared(self, X, y):
        ...
        return np.nan_to_num(X_t, False, np.finfo(np.float32).max, np.finfo(np.float32).max, np.finfo(np.float32).max)
