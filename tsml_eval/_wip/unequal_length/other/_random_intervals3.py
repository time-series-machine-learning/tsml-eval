...
class RandomIntervals(BaseCollectionTransformer):
    ...
    def _fit_setup(self, X):
        ...
        elif self.max_interval_length > self.min_n_timepoints_:
            self._max_interval_length = self.min_n_timepoints_
    ...
    def _generate_interval(self, X, y, seed, transform):
        ...
        if isinstance(X, list):
            g = []
            for x in X:
                d = int((self.max_n_timepoints_ - x.shape[1])/2)
                g.append(
                    np.expand_dims(x[dim, interval_start-d:interval_end-d:dilation], axis=0)
                )
        else:
            g = np.expand_dims(X[:, dim, interval_start:interval_end:dilation], axis=1)

        p = [True for _ in range(len(g))]
        for l in range(len(g)):
            if g[l].shape[1] < interval_length:
    ...
    def _transform_interval(self, X, idx, keep_transform):
        ...
        p = [True for _ in range(len(g))]
        for l in range(len(g)):
            if g[l].shape[1] < interval_end - interval_start:
                g[l] = np.zeros((1, self._min_interval_length))
                p[l] = False
