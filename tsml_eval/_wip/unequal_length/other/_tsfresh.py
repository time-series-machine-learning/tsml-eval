...
def _from_3d_numpy_to_long(arr):
    ...
    df = df[["index", "time_index", "column", "value"]]
    return df

def _from_np_list_to_long(arr):
    index = []
    time_index = []
    column = []
    value = []
    for i, case in enumerate(arr):
        for j, channel in enumerate(case):
            for k, timepoint in enumerate(channel):
                index.append(i)
                time_index.append(k)
                column.append(f"dim_{j}")
                value.append(timepoint)
...
class TSFresh(_TSFresh):
    ...
    def _transform(self, X, y=None):
        ...
        """
        Xt = _from_3d_numpy_to_long(X) if isinstance(X, np.ndarray) else _from_np_list_to_long(X)
