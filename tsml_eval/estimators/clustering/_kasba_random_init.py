...
class KASBAInit(BaseClusterer):
    ...
    def _fit(self, X: np.ndarray, y=None):
        ...
        cluster_centres, distances_to_centres, labels = self._init(X)
    ...
    def _assign_to_centers(self, X: np.ndarray, centers: np.ndarray):
        """Return (min_distances, labels) to given centers using self.distance."""
        curr_pw = pairwise_distance(
            X, centers, method=self.distance, **self._distance_params
        )
        labels = curr_pw.argmin(axis=1)
        distances_to_centers = curr_pw.min(axis=1)
        return distances_to_centers, labels

    def _random_center_initializer(self, X: np.ndarray):
        centers = X[
            self._random_state.choice(X.shape[0], self.n_clusters, replace=False)
        ]
        min_distances, labels = self._assign_to_centers(X, centers)
        return centers, min_distances, labels

    def _first_center_initializer(self, X: np.ndarray):
        """First k samples as centers; then assign."""
        centers = X[list(range(self.n_clusters))]
        min_distances, labels = self._assign_to_centers(X, centers)
        return centers, min_distances, labels
