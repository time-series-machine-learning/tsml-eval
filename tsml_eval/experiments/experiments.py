...
def run_classification_experiment(
    ...
    if build_test_file:
        ...
        if "composite" in classifier_name.lower():
...
def load_and_run_classification_experiment(
    ...
    run_classification_experiment(
        ...
        build_train_file=build_train,
        build_test_file=build_test,
    )
...
def run_clustering_experiment(
    ...
    build_time = int(round(time.time() * 1000)) - start
    start = int(round(time.time() * 1000))
    train_preds = clusterer.predict(X_train)
    build_time = int(round(time.time() * 1000)) - start
...
def _resample(train_X, train_y, test_X, test_y, random_state):
