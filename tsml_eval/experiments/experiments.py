...
def _device_description():
    """Describe the compute device an experiment ran on.

    Results files otherwise record nothing about the hardware, so a GPU result and a
    CPU result for the same estimator and dataset are indistinguishable by their
    contents even though their timings are not comparable.

    Returns
    -------
    str
        The GPU model if a deep learning framework is loaded and reports one, else
