...
class HydraMultiRocket(BaseClassifier):
    ...
    def _fit(self, X, y):
        ...
        X_training_transform_hydra = self.scale_hydra.fit_transform(X_training_transform_hydra)

        self.scale_multirocket = StandardScaler()
        X_training_transform_multirocket = self.scale_multirocket.fit_transform(X_training_transform_multirocket)

        X_training_transform_concatenated = \
        np.concatenate(
            (X_training_transform_hydra, X_training_transform_multirocket),
            axis = 1
        )
    ...
    def _predict(self, X) -> np.ndarray:
        ...
        X_test_transform_multirocket = self.scale_multirocket.transform(X_test_transform_multirocket)

        X_test_transform_concatenated = \
        np.concatenate(
            (X_test_transform_hydra, X_test_transform_multirocket),
            axis = 1
        )
