...
class Hydra(nn.Module):
    ...
    def __init__(self, ts_shape, k=8, g=64, max_num_channels=8):
        ...
        if self.n_dims_ > 1:
            ...
            self.I = [torch.randint(0, self.n_dims_, (divisor, _g, n_dims_per)) for _ in range(self.num_dilations)]
...
class HydraPipeline():
    ...
    def fit(self, X, y=None):
        self.transformation = Hydra(self.ts_shape, self.k, self.g, self.max_num_channels)
        return self
...
class HydraRegressor(BaseRegressor):
    ...
    def __init__(
        ...
        self.random_state = random_state.randint(np.iinfo(np.int32).max)
    ...
    def _fit(self, X, y):
        ...
        self.regressor = make_pipeline(HydraPipeline(ts_shape=X.shape, k=self.k, g=self.g, max_num_channels=self.max_num_channels), 
                                       StandardScaler(), 
