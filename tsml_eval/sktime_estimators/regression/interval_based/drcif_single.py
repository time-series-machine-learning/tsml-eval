...
class DrCIF(BaseRegressor):
    ...
    def _fit(self, X, y):
        ...
        elif isinstance(self.n_intervals, list) and len(self.n_intervals) == 1:
            self._n_intervals = self.n_intervals
        else:
            raise ValueError("DrCIF n_intervals must be an int or list of length 1.")
        ...
        elif isinstance(self.min_interval, list) and len(self.min_interval) == 1:
            self._min_interval = self.min_interval
        else:
            raise ValueError("DrCIF min_interval must be an int or list of length 1.")
        ...
        elif isinstance(self.max_interval, list) and len(self.max_interval) == 1:
            self._max_interval = self.max_interval
        else:
            raise ValueError("DrCIF max_interval must be an int or list of length 1.")
    ...
    def _predict_for_estimator(self, X, classifier, intervals, dims, atts):
        c22 = Catch22(outlier_norm=True)
        T = [X]
